"""
mef_viewer.py — IGI 2 MEF Viewer (Premium Edition — 100% Feature-Matched)
Entry point. 

Logic exactly synchronized with Variant1_StaticViewer, but powered by the official igi2mef library.
"""
...
try:
    ...
    try:
        ...
        print("ERROR: igi2mef library not found. Install with: pip install igi2mef"); sys.exit(1)
...
QTextEdit{background:#080814;border:none;color:#c0ccd8;font-family:Consolas,'Courier New',monospace;font-size:11px;padding:15px;}
"""
...
class _ParseWorker(QThread):
    ...
    def __init__(self, paths: List[Path], cache: Dict, debug_params: MefDebugParams):
        super().__init__(); self._paths = paths; self._cache = cache; self._debug = debug_params; self._cancelled = False
...
class _ThumbDelegate(QStyledItemDelegate):
    ...
    def paint(self, p: QPainter, opt, idx):
        ...
        if not d: return
        r = opt.rect; sel = bool(opt.state & QStyle.State_Selected)
        ...
        bx = tx + self.TW + self.PAD + 4; avail = r.right() - bx - self.PAD
        p.setPen(QColor("#dde6f0" if sel else "#c0ccd8")); p.setFont(QFont("Segoe UI", 8, QFont.Bold))
        name = QFontMetrics(p.font()).elidedText(d.get("name", ""), Qt.ElideMiddle, avail)
        p.drawText(bx, r.y() + 26, name); p.setPen(QColor("#607080")); p.setFont(QFont("Segoe UI", 7))
        p.drawText(bx, r.y() + 44, d.get("info", ""))

# ── Overlay flags ─────────────────────────────────────────────────────────────
class OverlayFlags:
    def __init__(self):
        self.bones = False; self.magic_verts = False; self.collision = False; self.portals = False; self.glow = False

# ── Cached overlays for one model ─────────────────────────────────────────────
class ModelOverlays:
    def __init__(self):
        self.bone_lines, self.bone_joints, self.magic_pts, self.coll_wire, self.portal_wire, self.glow_pts = None, None, None, [], None, None
...
class MEFViewport(QOpenGLWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._drag_btn = None; self._drag_last = QPoint(); self.wireframe, self.show_grid = False, True
        ...
        self.swizzles, self.bone_modes = ["XZY", "XYZ", "YXZ", "YZX", "ZXY", "ZYX"], ["REL", "ABS", "ROOT"]
        self.idx_vscale, self.idx_bscale, self.idx_swizzle, self.idx_bmode = 0, 0, 0, 0
        self.debug_params.v_scale = self.scales[self.idx_vscale]; self.debug_params.b_scale = self.scales[self.idx_bscale]
        self.debug_params.swizzle_mode = self.swizzles[self.idx_swizzle]; self.debug_params.bone_mode = self.bone_modes[self.idx_bmode]
    ...
    def initializeGL(self):
        ...
        self._grid_vao, self._grid_n = build_grid(self._ctx, self._flat_prog); self._gpu_cache = GpuCache(self._ctx, self._prog)
        sz = self.THUMB_SZ; tex = self._ctx.texture((sz, sz), 4); dep = self._ctx.depth_texture((sz, sz))
    ...
    def paintGL(self):
        w, h = self.width(), self.height(); qt_fbo = self._ctx.detect_framebuffer(self.defaultFramebufferObject())
        qt_fbo.use(); self._ctx.viewport = (0, 0, w, h); self._ctx.clear(0.04, 0.04, 0.10, 1.0)
        if self.current_model:
            mvp, eye = self.cam.matrices(w, h); self._prog["u_mvp"].write(gl_bytes(mvp))
            self._prog["u_model"].write(gl_bytes(np.eye(4, dtype="f4"))); self._prog["u_cam"].value = tuple(map(float, eye))
            self._ctx.wireframe = self.wireframe; gpu = self._gpu_cache.get(self.current_model)
...
class FileListPanel(QWidget):
    ...
    def __init__(self, viewport: MEFViewport, parent=None):
        super().__init__(parent); self._vp, self._cache, self._worker = viewport, {}, None
        layout = QVBoxLayout(self); layout.setContentsMargins(0, 0, 0, 0); layout.setSpacing(0)
        hdr = QWidget(); hdr.setFixedHeight(38); hdr.setStyleSheet("background:#060612;border-bottom:1px solid #1c1c38;")
        hl = QHBoxLayout(hdr); hl.setContentsMargins(10, 0, 10, 0); lbl = QLabel("MODELS"); lbl.setObjectName("sec")
        self._count = QLabel("0"); hl.addWidget(lbl); hl.addStretch(); hl.addWidget(self._count); layout.addWidget(hdr)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("IGI 2 MEF Viewer (Premium Edition)"); self.resize(1500, 880); self.setAcceptDrops(True)
        tb = self.addToolBar("Main"); tb.setMovable(False)
        tb.addAction("📂 Open", self._open_dir); tb.addAction("↺ Reload", self._reload); tb.addSeparator()

        self._act_wire = QAction("⬡ Wire", self, checkable=True); self._act_wire.toggled.connect(self._toggle_wire); tb.addAction(self._act_wire)
        self._act_grid = QAction("⊞ Grid", self, checkable=True, checked=True); self._act_grid.toggled.connect(self._toggle_grid); tb.addAction(self._act_grid)
        tb.addSeparator()

        self._act_bones = QAction("🦴 Bones", self, checkable=True); self._act_bones.toggled.connect(self._toggle_ov_bones); tb.addAction(self._act_bones)
        self._act_magic = QAction("✦ Magic", self, checkable=True); self._act_magic.toggled.connect(self._toggle_ov_magic); tb.addAction(self._act_magic)
        self._act_coll  = QAction("🔺 Coll",  self, checkable=True); self._act_coll.toggled.connect(self._toggle_ov_coll); tb.addAction(self._act_coll)
        self._act_portal= QAction("🚪 Portal", self, checkable=True); self._act_portal.toggled.connect(self._toggle_ov_portal); tb.addAction(self._act_portal)
        self._act_glow  = QAction("✨ Glow", self, checkable=True); self._act_glow.toggled.connect(self._toggle_ov_glow); tb.addAction(self._act_glow)
        tb.addSeparator()

        tb.addAction("⊙ Fit", self._fit_view); tb.addAction("⌂ Reset", self._reset_cam)
        ...
        self._file_panel.model_selected.connect(self._on_model_selected); self._vp.fps_changed.connect(self._on_fps)
        ...
        QShortcut(QKeySequence("B"), self, self._act_bones.toggle); QShortcut(QKeySequence("M"), self, self._act_magic.toggle)
        QShortcut(QKeySequence("C"), self, self._act_coll.toggle); QShortcut(QKeySequence("P"), self, self._act_portal.toggle)
        QShortcut(QKeySequence("F"), self, self._fit_view); QShortcut(QKeySequence("R"), self, self._reset_cam)
        QShortcut(QKeySequence("Ctrl+O"), self, self._open_dir)

    def _toggle_wire(self, on): self._vp.wireframe = on; self._vp.update()
    def _toggle_grid(self, on): self._vp.show_grid = on; self._vp.update()
    def _toggle_ov_bones(self, on): self._vp.overlays.bones = on; self._vp.update()
    def _toggle_ov_magic(self, on): self._vp.overlays.magic_verts = on; self._vp.update()
    def _toggle_ov_coll(self, on): self._vp.overlays.collision = on; self._vp.update()
    def _toggle_ov_portal(self, on): self._vp.overlays.portals = on; self._vp.update()
    def _toggle_ov_glow(self, on): self._vp.overlays.glow = on; self._vp.update()

    def _fit_view(self): 
        if self._vp.current_model: self._vp.cam.fit(self._vp.current_model.center, self._vp.current_model.radius); self._vp.update()
    def _reset_cam(self):
        self._vp.cam = OrbitCamera(); self._fit_view()

    def _open_dir(self):
        d = QFileDialog.getExistingDirectory(self, "Open MEF Folder", str(self._current_folder or Path.home()))
    ...
    def _on_model_selected(self, m: MefModel):
        self._vp.load_model(m); self._info_panel.show_model(m)
        pre = ""; 
        if m.bones: pre += f"🦴{len(m.bones)} "
        if m.magic_vertices: pre += f"✦{len(m.magic_vertices)} "
    ...
    def dropEvent(self, e):
        ...
        if urls:
            ...
            if p.is_file() and p.suffix.lower() == ".mef": self._load_folder(p.parent)
