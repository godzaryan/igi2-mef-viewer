...
try:
    from mef_parser_static import parse_mef, quick_validate, MefModel, MefDebugParams
except ImportError as e:
    print(f"ERROR: mef_parser_static not found or failed to load: {e}"); sys.exit(1)
...
class MEFViewport(QOpenGLWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.idx_vscale  = 0  # 0.01
        self.idx_bscale  = 0  # 0.01
        self.idx_swizzle = 0  # XZY
        self.idx_bmode   = 0  # REL
    ...
    def _draw_debug_hud(self):
        ...
        # Background
        p.fillRect(10, 10, 260, 230, QColor(0, 0, 0, 180))
        p.setPen(QColor("#00ff66")) # Green for verified
        p.setFont(QFont("Consolas", 9, QFont.Bold))
        p.drawText(20, 30, "✅ VERIFIED CONFIGURATION")
        
        p.setFont(QFont("Consolas", 8))
        def _line(y, label, val, shortcut):
            p.setPen(QColor("#8090a8"))
            p.drawText(20, y, f"{shortcut} {label}:")
            p.setPen(QColor("#ffffff"))
            p.drawText(140, y, f"{val}")

        d = self.debug_params
        _line(55,  "Vert Scale", f"{d.v_scale:.6f}", "[F1]")
        _line(75,  "Bone Scale", f"{d.b_scale:.6f}", "[F2]")
        _line(95,  "Swizzle",    f"{d.swizzle_mode}", "[F3]")
        _line(115, "Bone Mode",   f"{d.bone_mode}",    "[F4]")
        _line(135, "ID Bias",     f"{d.bone_id_bias}",  "[F5]")
        _line(165, "Flip X",      "ON" if d.flip_x else "off", "[F6]")
        _line(185, "Flip Y",      "ON" if d.flip_y else "off", "[F7]")
        _line(205, "Flip Z",      "ON" if d.flip_z else "off", "[F8]")
        
        p.setPen(QColor("#ffaa00"))
        p.drawText(20, 230, "Double-click search path to re-apply ↺")
        p.end()

    def keyPressEvent(self, e):
        # Experiment Key Controller
        if e.key() == Qt.Key_F1:
            self.idx_vscale = (self.idx_vscale + 1) % len(self.scales)
            self.debug_params.v_scale = self.scales[self.idx_vscale]
        elif e.key() == Qt.Key_F2:
            self.idx_bscale = (self.idx_bscale + 1) % len(self.scales)
            self.debug_params.b_scale = self.scales[self.idx_bscale]
        elif e.key() == Qt.Key_F3:
            self.idx_swizzle = (self.idx_swizzle + 1) % len(self.swizzles)
            self.debug_params.swizzle_mode = self.swizzles[self.idx_swizzle]
        elif e.key() == Qt.Key_F4:
            self.idx_bmode = (self.idx_bmode + 1) % len(self.bone_modes)
            self.debug_params.bone_mode = self.bone_modes[self.idx_bmode]
        elif e.key() == Qt.Key_F5:
            self.debug_params.bone_id_bias = (self.debug_params.bone_id_bias + 2) % 3 - 1
    ...
    def _refresh_model(self):
        ...
        # Clear GPU caches for THIS model
        if self._gpu_cache:
            self._gpu_cache._cache.pop(str(path), None)
        self._overlay_cache.pop(str(path), None)
        # Re-parse with debug params
        new_model = parse_mef(path, debug=self.debug_params)
        self.current_model = new_model
        # self.cam.fit(new_model.center, new_model.radius) # No fit, keep camera steady
...
class InfoPanel(QWidget):
    def __init__(self, parent=None):
        ...
        self._load_doc()
        tabs.addTab(self._doc_text, "MEF Spec 📖")

    def _load_doc(self):
        if _DOC_MD.exists():
            try:
                text = _DOC_MD.read_text(encoding="utf-8", errors="replace")
                self._doc_text.setPlainText(text)
            except Exception as e:
                self._doc_text.setPlainText(f"[Error loading documentation: {e}]")
        else:
            self._doc_text.setPlainText(
                "Documentation not found.\n"
                f"Expected: {_DOC_MD}\n\n"
                "Place MEF_text.md in the MEFInfo directory."
            )
